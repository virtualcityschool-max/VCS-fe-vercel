      ...
      {isOpen ? (
        <div className="w-80 h-96 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden glass">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {messages.length === 0 && (
              <p className="text-slate-400 text-sm italic">
                Ask me anything about the {currentView} page!
              </p>
            )}
            {messages.map((m, i) => (
              <div
              ...
              >
                <div
                  className={`max-w-[80%] p-2 rounded-lg text-sm ${m.role === "user" ? "bg-indigo-600 text-white" : "bg-slate-800 text-slate-200"}`}
                >
                  {m.text}
                </div>
              </div>
          ...
          <div className="p-3 border-t border-slate-700 flex gap-2">
            ...
            <button
              onClick={handleSend}
              className="bg-indigo-600 text-white px-3 py-2 rounded-lg hover:bg-indigo-500 transition"
            >
