    ...
    return (
      <div className="bg-slate-800 p-8 rounded-[2.5rem] border border-slate-700 shadow-xl">
        ...
        <div className="text-center py-8">
          ...
          <h4 className="text-lg font-semibold text-white mb-2">No Assignments</h4>
          <p className="text-slate-500 text-sm">You don't have any assignments yet</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-slate-800 p-8 rounded-[2.5rem] border border-slate-700 shadow-xl">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {Object.entries(statusCounts).map(([status, count]) => {
          ...
          return (
            <div key={status} className={`${config.color} p-3 rounded-xl text-center`}>
              <div className="text-2xl font-bold mb-1">{count}</div>
              <div className="text-xs font-black uppercase tracking-widest">{config.label}</div>
            </div>
          );
        })}
      </div>
      ...
      {assignments.length > 6 && (
        <button className="w-full mt-8 py-3 bg-slate-900 border border-slate-700 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition active:scale-95">
