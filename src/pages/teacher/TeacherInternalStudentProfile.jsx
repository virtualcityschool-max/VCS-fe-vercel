...
const TeacherInternalStudentProfile = () => {
  ...
  return (
    <section
      id="student-risk-view"
      className="min-h-screen bg-[#0f172a] text-white font-inter p-4 md:p-8"
    >
      {/* Navigation Header */}
          ...
          <div className="w-full md:w-80 shrink-0">
            <div className="bg-slate-800/50 backdrop-blur-md rounded-[2.5rem] p-8 border border-slate-700 shadow-2xl text-center">
              <div className="relative inline-block mb-6">
                <img
                  src="https://i.pravatar.cc/150?u=stud123"
                  className="w-32 h-32 rounded-3xl border-4 border-slate-700 shadow-xl object-cover"
                  alt="Kevin Wu"
                />
                <div
                  className="absolute -bottom-2 -right-2 w-8 h-8 bg-red-500 border-4 border-slate-800 rounded-full shadow-lg"
                  title="Critical Status"
                ></div>
              </div>
              <h2 className="text-2xl font-black font-poppins text-white mb-1">
                Kevin Wu
              </h2>
          ...
          <div className="flex-1 space-y-8">
            ...
            <div className="bg-slate-800/50 backdrop-blur-md p-8 rounded-[2.5rem] border border-slate-700 shadow-xl">
              <h3 className="text-xl font-bold font-poppins text-white mb-8 flex items-center gap-3">
                <i className="fas fa-shield-alt text-indigo-400"></i> Behavioral
                Risk Assessment
              </h3>
                ...
                {[
                  {
                    ...
                    note: "Student showed exceptional insight during the React session. However, failed to cite sources in the final lab report.",
                    type: "Warning",
                    color: "bg-red-500",
                  },
                  {
                    date: "Sep 28, 2023",
                    teacher: "Prof. Mark Wood",
                    note: "Kevin is struggling with participation. Needs encouragement during group activities.",
                    type: "Observation",
                    color: "bg-indigo-500",
                  },
                ].map((n, i) => (
                  <div
                    key={i}
                    className="bg-slate-900/40 p-6 rounded-3xl border border-slate-700/50 hover:bg-slate-900 transition"
                  >
                    <div className="flex justify-between items-center mb-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-1 h-8 rounded-full ${n.color}`}
                        ></div>
                      ...
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full text-white shadow-lg ${n.color}`}
                      >
                        {n.type}
                      </span>
                    </div>
            ...
            <div className="bg-slate-800/50 backdrop-blur-md p-8 rounded-[2.5rem] border border-slate-700 shadow-xl">
              <div className="flex justify-between items-center mb-10">
                <h3 className="text-xl font-bold font-poppins text-white flex items-center gap-3">
                  <i className="fas fa-chart-bar text-green-500"></i> Grade
                  Distribution
                </h3>
                <div className="flex gap-2">
                  <button className="bg-slate-900 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase text-slate-500 border border-slate-700">
                    MT1
                  </button>
                  <button className="bg-indigo-600 px-4 py-1.5 rounded-xl text-[10px] font-black uppercase text-white shadow-lg shadow-indigo-900/20">
                    Finals
                  </button>
                </div>
              </div>
              <div className="h-64 flex items-end gap-6 px-4">
                {[80, 45, 95, 70, 85].map((h, i) => (
                  <div
                    key={i}
                    className="flex-1 bg-slate-900/50 rounded-2xl relative group cursor-pointer border border-slate-800"
                  >
                    <div
