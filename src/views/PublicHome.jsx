...
const PublicHome = ({ setView, setAuthModal, auth, onLogout }) => {
  const getAvatarUrl = () => {
    ...
    <section id="public-home" className="min-h-screen bg-[#020617] text-white selection:bg-indigo-500/30 overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex justify-between items-center">
          <div className="flex items-center gap-4 sm:gap-12">
            ...
            <div className="hidden lg:flex items-center gap-8">
              <button onClick={() => setView(AppView.MARKETPLACE)} className="text-sm font-bold text-slate-400 hover:text-white transition">Browse Courses</button>
              <button onClick={() => setView(AppView.INSTRUCTORS_DIRECTORY)} className="text-sm font-bold text-slate-400 hover:text-white transition">Find Tutors</button>
            </div>
          </div>
            ...
            {!auth.isLoggedIn ? (
              ...
              <div id="nav-user" className="flex items-center gap-6 animate-fadeIn">
                <div className="flex items-center gap-4 group cursor-pointer" onClick={() => {
                   if(auth.role === 'student') setView(AppView.STUDENT);
                   if(auth.role === 'teacher') setView(AppView.TEACHER);
                   if(auth.role === 'admin') setView(AppView.ADMIN);
                   if(auth.role === 'parent') setView(AppView.PARENT);
                }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-40 text-center">
        ...
        <div className="max-w-3xl mx-auto relative mb-16 group">
          ...
          <div className="relative flex flex-col md:flex-row gap-4 p-2 bg-slate-900/80 backdrop-blur-2xl border border-white/10 rounded-4xl shadow-2xl">
             ...
             <button onClick={() => setView(AppView.MARKETPLACE)} className="bg-indigo-600 hover:bg-indigo-500 text-white px-10 py-4 rounded-4xl font-black text-xs uppercase tracking-widest transition shadow-xl shadow-indigo-900/20">
               Search Catalog
             </button>
          </div>
        </div>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-6 py-32 border-t border-white/5">
        <div className="flex justify-between items-end mb-16">
          ...
          <button onClick={() => setView(AppView.MARKETPLACE)} className="text-indigo-400 font-black text-[10px] uppercase tracking-widest hover:text-white transition flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {[
            ...
            <div key={i} onClick={() => setView(AppView.MARKETPLACE)} className="bg-slate-900/50 border border-white/10 rounded-[2.5rem] overflow-hidden group cursor-pointer hover:border-indigo-500/50 transition-all shadow-xl flex flex-col">
