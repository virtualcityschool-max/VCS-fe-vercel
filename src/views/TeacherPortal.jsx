...
const TeacherPortal = ({ setView }) => {
  ...
  return (
    <section id="teacher-view" className="min-h-screen bg-slate-950 text-white flex">
      {/* Mobile Header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-slate-950 border-b border-slate-800 z-100 flex items-center justify-between px-6">
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <span className="text-[10px] sm:text-sm font-black tracking-tighter whitespace-nowrap">VirtualCitySchool</span>
        </div>
      ...
      <aside className={`w-72 bg-slate-950 border-r border-slate-800 flex flex-col fixed h-full z-50 transition-transform duration-300 lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-10">
          ...
          <nav className="space-y-2">
            {['Dashboard', 'My Classes', 'Attendance', 'Grading', 'AI Assistant'].map(item => (
              <button key={item} className="w-full text-left px-6 py-4 rounded-2xl text-xs font-bold uppercase tracking-widest text-slate-500 hover:bg-slate-900 hover:text-white transition">
                {item}
              </button>
            ))}
          </nav>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          <div className="lg:col-span-2 bg-linear-to-br from-indigo-600 to-indigo-800 p-8 sm:p-10 rounded-5xl shadow-2xl relative overflow-hidden">
            <div className="relative z-10">
              <h2 className="text-2xl sm:text-3xl font-black font-poppins mb-2 text-white">Welcome, Dr. Elena!</h2>
              <p className="text-indigo-100 text-sm sm:text-base font-medium">You have <span className="underline decoration-white/30">3 Live Classes</span> starting in the next 2 hours.</p>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 p-8 rounded-5xl flex flex-col justify-center text-center">
            <p className="text-[10px] font-black uppercase text-slate-500 tracking-[0.3em] mb-4">Total Students</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-10">
          <div className="lg:col-span-6 space-y-8">
            ...
            <div className="space-y-4">
              {[
                { time: '10:00 AM', title: 'Quantum Mechanics Basics', room: 'VR-102', count: 42 },
                { time: '01:30 PM', title: 'Advanced Lab: Optics', room: 'VR-005', count: 12 },
              ].map((c, i) => (
                <div key={i} className="bg-slate-900 p-6 rounded-3xl border border-slate-800 flex items-center justify-between hover:border-indigo-500 transition cursor-pointer group">
                  <div className="flex items-center gap-6">
                    <div className="text-indigo-400 font-black text-sm">{c.time}</div>
                    <div>
                      <p className="font-bold text-white group-hover:text-indigo-400 transition">{c.title}</p>
                      <p className="text-[10px] text-slate-500 uppercase tracking-widest">{c.room} • {c.count} Learners</p>
                    </div>
                  </div>
            ...
            <div className="bg-rose-500/5 border border-rose-500/20 p-8 rounded-5xl space-y-6 shadow-2xl">
              {[
                ...
                <div key={i} onClick={risk.action} className="flex items-center justify-between p-4 bg-slate-900 rounded-2xl border border-rose-500/20 cursor-pointer hover:bg-rose-500/10 transition group">
                   <div>
                     <p className="font-bold text-white group-hover:text-rose-400">{risk.name}</p>
                     <p className="text-[10px] text-rose-500 uppercase font-black tracking-widest">{risk.status}</p>
                   </div>
                   <button className="bg-rose-600 text-white px-4 py-2 rounded-xl text-[8px] font-black uppercase shadow-lg">Review</button>
                </div>
              ))}
            </div>
          </div>
